...
class hadoop ($hadoop_security_authentication = "simple",
  ...
  $proxyusers = {
    oozie => { groups => 'hudson,testuser,root,hadoop,jenkins,oozie,hive,httpfs,users,yarn,impala,srvc.ldm.fu', hosts => "*" },
     hive => { groups => 'hudson,testuser,root,hadoop,jenkins,oozie,hive,httpfs,users,yarn,impala,srvc.ldm.fu', hosts => "*" },
     yarn => { groups => 'hudson,testuser,root,hadoop,jenkins,oozie,hive,httpfs,users,yarn,impala,srvc.ldm.fu', hosts => "*" },
     httpfs => { groups => 'hudson,testuser,root,hadoop,jenkins,oozie,hive,httpfs,users,yarn,impala,srvc.ldm.fu', hosts => "*" },
    impala => { groups => 'hudson,testuser,root,hadoop,jenkins,oozie,hive,httpfs,users,yarn,impala,srvc.ldm.fu', hosts => "*" } },
  ...
  class common_hdfs ($ha = "manual",
      $hadoop_config_dfs_block_size = 536870912,
      ...
      $hadoop_namenode_https_port = "50070",
